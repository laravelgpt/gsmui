...
class GSMUIComponentCommand extends Command
{
    /**
    ...
    public function handle()
    {
        ...
        if ($type === 'all' || $type === 'blade') {
            if ($this->createBladeComponent($studlyName, $slug, $kebabName, $category, $variant, $size, $force)) {
                $created[] = "Blade: resources/views/vendor/gsmui/components/blade/{$category}/{$slug}.blade.php";
            }
        }
        ...
        if ($this->createDocumentation($studlyName, $slug, $kebabName, $category, $variant, $size, $force)) {
            $created[] = "Docs: resources/views/vendor/gsmui/components/docs/{$slug}.md";
        }
        ...
        $this->info("\n💡 Usage examples:");
        $this->info("   Blade: <x-gsmui::components.{$category}.{$slug} />");
    ...
    {
        $path = resource_path("views/vendor/gsmui/components/blade/{$category}/{$slug}.blade.php");
    ...
    {
        $path = resource_path("views/vendor/gsmui/components/docs/{$slug}.md");
...
### Blade
```blade
<x-gsmui::components.{$category}.{$slug} 
    label="Get Started" 
    variant="primary" 
    size="lg" 
    icon="arrow-right"
...
### Primary Button
```blade
<x-gsmui::components.{$category}.{$slug} label="Primary Action" />
```
